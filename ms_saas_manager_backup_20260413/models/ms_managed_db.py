...
class MsManagedDb(models.Model):
    ...
    def provision(self, subscription, admin_email, admin_password, language='en_US'):
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                self.write({'state': 'error', 'error_message': error})
                _logger.error('Provisioning failed for %s: %s', db_name, error)
                return False

            # Set admin credentials via shell
            set_admin_cmd = (
                f'/opt/odoo/venv/bin/python3 '
                f'/opt/odoo/server/odoo-bin shell '
                f'-c {config_path} '
                f'-d {db_name} '
                f'--no-http << \'PYEOF\'\n'
                f'u = env["res.users"].browse(2)\n'
                f'u.write({{"login": "{admin_email}", "password": "{admin_password}"}})\n'
                f'env.cr.commit()\n'
                f'PYEOF'
            )
            subprocess.run(set_admin_cmd, shell=True, timeout=60)
    ...
    def action_delete_database(self):
        ...
        try:
            ...
            # Drop database
            result = subprocess.run(
                f'psql -U odoo -d ms_master -c "DROP DATABASE IF EXISTS \"{db_name}\";"',
